...
static int ShellAppRun(PUserContext, int, char**);

const UserApp ShellApp = { "shell", ShellAppRun, "", "Application shell" };

int ShellAppRun(PUserContext Context, int argc, char** argv)
{
	int rc = 0;
	...
	if ((rc = console_tokenize(Context, command)) >= 0)
	{
		if (Context->argc > 0)
		{
			...
			{
				// Look up a command.
				if(strcmp(AppList[i]->AppName, Context->argv[0]) == 0)
				{
					if ((rc = console_exec(Context, AppList[i])) < 0)
					{
