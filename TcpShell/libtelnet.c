...
static INLINE void _set_rfc1143(telnet_t *telnet, unsigned char telopt,
	 ...
	 */
	telnet_rfc1143_t *qtmp2 = pvPortMalloc(sizeof(telnet_rfc1143_t) * (telnet->q_size + 4));
	if (!qtmp2)
	{
		_error(telnet,
			__LINE__,
	...
	memcpy(qtmp2, telnet->q, telnet->q_size);
	memset(&qtmp2[telnet->q_size], 0, sizeof(telnet_rfc1143_t) * 4);
	if (telnet->q)
	{
		vPortFree(telnet->q);
	}
...
static int _environ_telnet(telnet_t *telnet, unsigned char type,
	...
	if ((values = (struct telnet_environ_t *)pvPortMalloc(count * sizeof(struct telnet_environ_t))) == 0) {
...
static int _zmp_telnet(telnet_t *telnet, const char* buffer, size_t size) {
	...
	/* allocate argument array, bail on error */
	if ((argv = (char **)pvPortMalloc(argc * sizeof(char *))) == 0) {
		_error(telnet, __LINE__, __func__, TELNET_ENOMEM, 0,
				"calloc() failed: %s", strerror(errno));
		return 0;
	}
...
static int _ttype_telnet(telnet_t *telnet, const char* buffer, size_t size) {
	...
	if (buffer[0] == TELNET_TTYPE_IS) {
		...
		if ((name = (char *)pvPortMalloc(size)) == 0) {
			_error(telnet, __LINE__, __func__, TELNET_ENOMEM, 0,
					"malloc() failed: %s", strerror(errno));
			return 0;
		}
...
telnet_t *telnet_init(const telnet_telopt_t *telopts,
	...
	/* allocate structure */
	struct telnet_t *telnet = (telnet_t*)pvPortMalloc(1 * sizeof(telnet_t));
	if (telnet == 0)
		return 0;
...
static telnet_error_t _buffer_byte(telnet_t *telnet,
	...
	if (telnet->buffer_pos == telnet->buffer_size) {
		...
		/* (re)allocate buffer */
		new_buffer = (char *)pvPortMalloc(_buffer_sizes[i + 1]);
		if (new_buffer == 0) {
			_error(telnet, __LINE__, __func__, TELNET_ENOMEM, 0,
					"pvPortMalloc() failed");
		...
		memcpy(new_buffer, telnet->buffer, telnet->buffer_size);
		if (telnet->buffer)
		{
			vPortFree(telnet->buffer);	
		}
